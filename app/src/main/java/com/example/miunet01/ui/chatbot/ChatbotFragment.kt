...
class ChatbotFragment : Fragment() {
    ...
    //  Base de conocimiento inicial (se puede pasar a Firebase más adelante)
    ...
    private fun obtenerRespuesta(pregunta: String): String {
        ...
        // Base de conocimiento mejorada para el chatbot UNET
         val baseConocimientoAvanzada = mapOf(
            ...
            listOf("informática", "computación", "sistemas", "programación") to listOf(
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        val textos = mensajes.map { it.texto }
        val autores = mensajes.map { it.esUsuario }

        outState.putStringArrayList("mensajes_texto", ArrayList(textos))
        outState.putBooleanArray("mensajes_autor", autores.toBooleanArray())
    }

    override fun onViewStateRestored(savedInstanceState: Bundle?) {
        super.onViewStateRestored(savedInstanceState)

        val textos = savedInstanceState?.getStringArrayList("mensajes_texto")
        val autores = savedInstanceState?.getBooleanArray("mensajes_autor")

        if (textos != null && autores != null) {
            mensajes.clear()
            for (i in textos.indices) {
                mensajes.add(Mensaje(textos[i], autores[i]))
            }
